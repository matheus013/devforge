...
def request_claude_code_handoff(*, project, objective: str, requested_by) -> AgentRun:
    return AgentRun.objects.create(
        ...
        output={
            "handoff": (
                "Claude Code deve atuar localmente no workspace, manter isolamento por "
                "organizacao, registrar decisoes no projeto e nao executar codigo importado."
            ),
            "next_step": (
                "Abrir a fila de AgentRun com status pending_claude_code e executar a tarefa."
            ),
        },
        estimated_cost="0.00",
        logs=[
            agent_log_context("claude-code-local-operator"),
            "Solicitacao criada para atuacao do Claude Code no ambiente local.",
        ],
    )


@transaction.atomic
def resolve_project_with_claude_code(*, project: Project, objective: str, requested_by) -> AgentRun:
    tickets = [
        (
            "Claude Code: consolidar requisitos operacionais",
            "Transformar o briefing atual em criterios de aceite testaveis por tela e fluxo.",
            "high",
        ),
        (
            "Claude Code: preparar backlog da proxima entrega",
            "Quebrar o projeto em tickets pequenos para frontend, backend, dados e validacao.",
            "normal",
        ),
        (
            "Claude Code: revisar riscos de isolamento",
    ...
    next_actions = [
        ...
        "Executar tickets Claude Code em ordem de prioridade.",
        "Manter imports apenas como assessment estrutural ate existir sandbox real.",
    ]
    summary = (
        f"Claude Code local analisou {project.name} e preparou uma fila operacional "
    ...
    deployment = publish_subscription_deployment(
        project=project,
        notes="Implantacao de assinatura publicada pelo Claude Code local para cliente e time.",
    )
    if all_stages_done:
    ...
    return AgentRun.objects.create(
        ...
        logs=[
            agent_log_context("claude-code-local-operator"),
            "Projeto analisado pelo Claude Code local.",
